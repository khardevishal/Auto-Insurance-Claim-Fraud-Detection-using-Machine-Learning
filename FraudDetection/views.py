...
def processClaim(request):
    if request.method == "POST":
        ...
        accidentLoc = request.POST['accidentLoc']
        name = request.POST['name']
        policeReport = request.POST['policeReport']
        ...
        if(eval(ag) < 18 or eval(rating) < 2 or eval(numberCars) < 0 or eval(policeReport) == 0):
            strML = 'Rejected'
        else:    
            if(result == 0):
                strML = 'Approved'
        ...
        en = claimQueryModel(service_username=userEm, service_sex=sx,service_maritalStatus=marrytal,service_age=ag, 
                             ...
                             service_ML_Output=strML,service_final_output=strGarage, service_accidentLoc=accidentLoc,
                             service_policy=policyNumber,service_name=name,service_police_report=policeReport)
        send_mail(
...
Machine Learning Output:""" + i.service_ML_Output +
